...
var tml = {
  ...
  initApplication: function(token, options, callback) {
    ...
    app.init(options, function (err) {
      ...
      tml.domReady(function(){
        ...
        if (typeof(tml.options.onLoad) == "function") {
          tml.options.onLoad(app);
        }
  ...
  changeLanguage: function(locale) {
    app.changeLanguage(locale, function(language) {
      ...
      //}
      tml.updateLanguageSelector();
      if (typeof(tml.options.onLanguageChange) == "function")
    ...
    var moHandler = function(mutations) {
      ...
      if (mutations.length > 0) {
        mutations.forEach(function(mutation) {
          var target = mutation.target;
          var nodes = mutation.addedNodes || [];
          if(self.tokenizer.isNoTranslate(target) || !document.body) return;
          nl.push(Array.prototype.slice.call(nodes));
        });
        if(document.body){
          observer.disconnect();
          nl.forEach(function(nodes){
            self.tokenizer.translateDOM(n);
          });
          observer.observe(document.body, moConfig);
        }
      }
    };
