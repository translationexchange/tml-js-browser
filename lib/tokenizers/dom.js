...
DomTokenizer.prototype = {
  ...
  translateDOM: function(node) {
    ...
    if (node.tagName === 'A') {
      if (window.tml) {
        var strategy;
        if (tml.config && tml.config.locale_strategy) {
          strategy = tml.config.locale_strategy;
        }

        if (strategy)
          dom.applyLocaleToLink(node, strategy, helpers.getCurrentLocale(tml.config.key, tml.config.current_locale))
      }
    }
    var buffer = [];
